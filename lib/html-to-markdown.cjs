...
function htmlToMarkdown(html) {
  ...
  // Blog posts have no <main>, so we fall back to <body> and the mobile menu
  // (a <div>, not a <nav>) would otherwise prepend the nav links to every post.
  s = s.replace(/<div[^>]*class="[^"]*mobile-menu[^"]*"[^>]*>[\s\S]*?<\/div>/gi, '');
