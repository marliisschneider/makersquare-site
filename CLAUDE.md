...
Pricing: Early bird $1,199 / Regular $1,399. Attendees get $200 off the January 2-week program (code WORKSHOP200). Private team sessions: custom quote only — never publish a price (~$950/person internally, min ~$4,000). **Stripe: all 6 payment links are LIVE (early-bird + regular for Sep, Oct, and Nov) — created Jul 31, 2026. workshop-register.html auto-switches EB→regular by date for all three. The "Reserve a seat" buttons on /workshops link **directly to /workshop-register** (Stripe) — changed Sep 18 2026 because the Zoho form's after-submit redirect was broken and trapping mobile users. Restore the lead-capture form only after that redirect is fixed Zoho-side.** Still true: no workshops on the enrollment page.
